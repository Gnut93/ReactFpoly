...
import roundToThousand from "../function/function";
          ...
          .map((sp, i) => {
            return (
              <div className="sp" key={i}>
                ...
                <h4 className="ten">{sp.ten_sp}</h4>
                <p className="gia">
                  {roundToThousand(parseFloat(sp.gia_km)).toLocaleString(
                    "vi-VI"
                  )}
                  VNĐ
                </p>
                ...
                >
                  {roundToThousand(parseFloat(sp.gia)).toLocaleString("vi-VI")}
                  VNĐ
                </p>
          ...
          .map((sp, i) => {
            return (
              <div className="sp" key={i}>
                ...
                <h4>{sp.ten_sp}</h4>
                <p className="gia">
                  {roundToThousand(parseFloat(sp.gia_km)).toLocaleString("vi-VI")}VNĐ
                </p>
                <p
                  className="gia-km"
                  style={{ textDecoration: "line-through" }}
                >
                  {roundToThousand(parseFloat(sp.gia).toLocaleString("vi-VI")}VNĐ
                </p>
                <p className="luotxem">Lượt xem: {sp.soluotxem}</p>
                <button className="btn">Đặt Hàng</button>
                <button className="btn">
